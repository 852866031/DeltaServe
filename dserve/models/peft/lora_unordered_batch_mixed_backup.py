...
class LoraUnorderedBatchMixed:
    ...
    def _context_forward(self, input_ids, infer_state, no_lora_compute=False, prefill_interrupt_event=None):
        ...
        input_embs_layer_end_time = time.time()
        ...
        for i in range(self.base_model.layers_num):
            ...
            #if i==16: print(f"Layer {i} attention time: {time.time() - att_start:.4f}s")
            ...
            #if i==16: print(f"Layer {i} FFN time: {time.time() - ffn_start:.4f}s")   
            if self.interrupt_and_clean(prefill_interrupt_event, infer_state, FFN_input_vpids, attention_input_vpids): return None
            if torch.any(infer_state.finetune_mask):
        ...
        transformer_layer_end_time = time.time()
        if self.interrupt_and_clean(prefill_interrupt_event, infer_state, FFN_input_vpids, attention_input_vpids): return None
        if infer_state.ref_mask is not None:
        ...
        output_layer_end_time = time.time()
    ...
    def _token_forward(self, input_ids, infer_state, no_lora_compute=False, no_lora_copy=False, print_time_profile=True):
        ...
        for i in range(self.base_model.layers_num):
            ...
            input_embs, attn_time, norm_time, pre_cache_time, get_qkv_time, post_cache_time, atten_kernel_time, get_o_time, \
                pin_adapter_time, export_lora_param_time, o_kernel_time, unpin_adapter_time, translation_time = \
        ...
        if print_time_profile:
            ...
            print(f"[forward engine]:\t\t\t\ttranslation time {total_translation_time:.5f}\n")
    ...
    def _lora_token_forward(self, layer_id, input_embs, infer_state, no_lora_compute=False, no_lora_copy=False):
        ...
        # mark_end("token_ffn")
        return input_embs, attention_time-start, norm_time, pre_cache_time, get_qkv_time, post_cache_time, atten_kernel_time, get_o_time, pin_adapter_time, export_lora_param_time, o_kernel_time, unpin_adapter_time, translation_time
    ...
    def _lora_token_attention(self, layer_id, input_embs, infer_state, no_lora_compute=False, no_lora_copy=False):
        ...
        if self.enable_unified_mem_manager:
            q, translation_time = self._batch_lora_get_qkv_alt(layer_id, input1, cache_k, cache_v, infer_state, no_lora_compute, no_lora_copy)
            ...
            post_cache_time = post_cache_end - time1
        ...
        input_embs.add_(o.view(-1, layer_infer.embed_dim_))
        return norm_time, pre_cache_time, get_qkv_time, post_cache_time, atten_kernel_time, get_o_time, pin_adapter_time, export_lora_param_time, o_kernel_time, unpin_adapter_time, translation_time
    

    # @calculate_time(show=True, min_cost_ms=0)
    # this impl dont to use @mark_cost_time
    def _lora_token_attention_timing(self, layer_id, input_embs, infer_state, no_lora_compute=False, no_lora_copy=False):
        start = time.time()
        layer_weight = self.base_model.trans_layers_weight[layer_id]
        layer_infer = self.base_model.layers_infer[layer_id]
        # layer normalization
    ...
    def _batch_lora_get_o_alt(self, layer_id, input, infer_state, no_lora_compute=False)->torch.Tensor:
        ...
        if not no_lora_compute:
            ...
            export_lora_param_time = export_lora_param_end - start
            delta_oA = self.delta[0]
            dispatch_bgmv(delta_oA, input.view(-1, base_layer_infer.embed_dim_), 
                            buffer_address, 
                            a_start_lora, a_len_lora, 
        ...
        return o, 0, export_lora_param_time, o_kernel_time, unpin_adapter_time