#!/usr/bin/env python3
"""
Exp 2 — Predictor Sweep: Llama3-8B with REAL Weights
=====================================================
Same predictor evaluation as exp1_5 / sweep_mixtral.py, but for Llama3-8B.

Llama3 is a *dense* transformer (no MoE routing).  The sweep serves as the
control arm: we expect the predictor to work well at all context lengths and
batch sizes because Σn² and T_in fully characterise the compute.  Any error
here is from measurement noise only, not structural under-specification.

Comparing Llama3 results with Mixtral results from sweep_mixtral.py reveals:
  • How much of Mixtral's predictor error comes from routing variance
    vs. ordinary timing noise

Runs on a single GPU — Llama3-8B fits entirely in one A100-80GB.
All dist.all_reduce calls in the model are guarded by `world_size > 1`,
so no process group is needed.

Usage:
    cd S-LoRA
    python test/exp2/sweep_llama3.py \\
...
# Skip configs with more total tokens than this
MAX_TOKENS = 200_000
# KV cache pool (tokens).
# Llama3-8B on 1 GPU: 8 KV heads × 128 dim × 32 layers × fp16 = 64 KB/token
# 80 GB - 16 GB model ≈ 64 GB free → 1M tokens available; 80K is very conservative.
MAX_POOL = 80_000
...
def run_prefill_then_decode(model, token_ids_list):
    ...
    input_ids_p = torch.from_numpy(flat).to(DEVICE)
    ...
    # Decode (one step on the same KV cache)
    b_loc_d     = torch.cat([b_loc, torch.zeros(bs, 1, dtype=torch.long, device=DEVICE)], dim=1)
    b_seq_len_d = b_seq_len + 1
    total_d     = int(b_seq_len_d.sum().item())
    max_len_d   = int(b_seq_len_d.max().item())
    input_ids_d = torch.randint(0, model.vocab_size, (bs,), dtype=torch.long, device=DEVICE)
...
def run_sweep(model_dir):
    torch.cuda.set_device(0)

    from slora.models.llama3.model import Llama3TpPartModel

    print(f"Loading Llama3 weights from: {model_dir}")
    ...
    print(f"Loaded.  vocab_size={vocab_size}  "
          f"layers={model.layers_num}  "
          f"kv_heads/rank={model.tp_k_head_num_}  "
          f"head_dim={model.head_dim_}\n")
    ...
    skipped = [(bs, sl) for sl in CONTEXT_LENGTHS for bs in BATCH_SIZES
               if bs * sl > MAX_TOKENS]
    print(f"{len(configs)} configs to run, {len(skipped)} skipped (bs×sl > {MAX_TOKENS})")
    ...
    for cfg_idx, (bs, sl) in enumerate(configs):
        ...
        print(f"[{cfg_idx+1:2d}/{len(configs)}] {label} | "
              ...
              f"cv={r['prefill_cv_pct']:.1f}% err={r['prefill_mean_err_pct']:.1f}% | "
...
def main():
    ...
    parser.add_argument("--model_dir", default="meta-llama/Meta-Llama-3-8B",
                        help="HuggingFace repo id or local path to Llama3 checkpoint")
    args = parser.parse_args()

    if torch.cuda.device_count() < 1:
        print("Need at least 1 GPU"); sys.exit(1)

    sweep = run_sweep(args.model_dir)
    print_tables(sweep)
    save_results(sweep, OUT_DIR, model_tag="llama3_real")
    print("\nDone.")
