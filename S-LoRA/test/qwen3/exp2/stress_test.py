#!/usr/bin/env python3
"""
Experiment 2 — Qwen3-30B-A3B Predictor Stress Test
====================================================
...
Requires 2 GPUs (EP mode) and Qwen3-30B-A3B weights.
...
def exp_b(model, vocab_size, rank):
    ...
    results = []
    if rank == 0:
        arr = np.array(all_decode_times)
        mean_times = arr.mean(axis=0)
    ...
    if rank == 0:
        ...
        for slack_ms in SLO_SLACKS_MS:
            ...
            for pred_t, actual_t in zip(preds, test_times):
                ...
                elif   gate_admit and not true_admit: fp += 1
                elif not gate_admit and   true_admit: fn += 1
                else:                                 tn += 1
