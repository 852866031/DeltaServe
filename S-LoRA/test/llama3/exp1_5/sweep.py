#!/usr/bin/env python3
"""
Experiment 1.5 — Extended Uniform Config Sweep (Llama3)
========================================================
Tests PrefillExecutionEstimator AND DecodeExecutionEstimator across:
  - Context lengths: 256, 512, 1024, 2048, 4096, 8192
  - Batch sizes:     1, 2, 4, 8, 16, 32, 64, 128

Llama3 is a dense transformer (no MoE), so there is no expert-routing
variance.  The predictor should converge; this experiment measures HOW
well it converges across the same config grid used in the Mixtral sweep,
providing a clean baseline for comparison.

For each (batch_size, context_len) pair:
  1. Run N prefill passes → different input_ids each time → time varies
...
Usage:
    cd S-LoRA
    python test/llama3/exp1_5/sweep.py
"""
...
# ---------------------------------------------------------------------------
# Config — tiny GQA model, same context-length ceiling as Mixtral sweep
# ---------------------------------------------------------------------------
TINY_CONFIG = {
    "model_type": "llama3",
    "hidden_size": 1024,
    ...
    "num_key_value_heads": 2,       # GQA: 4 Q-heads per KV-head
    "num_hidden_layers": 4,
    "vocab_size": 4096,
    "rms_norm_eps": 1e-5,
...
# ---------------------------------------------------------------------------
# Main sweep (single GPU — no dist needed for world_size=1)
# ---------------------------------------------------------------------------
...
def run_sweep():
    ...
    # Subclass to override _init_config: dummy mode in the basemodel calls
    # get_config_json(name) which is a name registry — it doesn't read from disk.
    # We always want to load from the tempdir config.json.
    class Llama3TpPartModelFromDir(Llama3TpPartModel):
    ...
    config_dir = tempfile.mkdtemp(prefix="llama3_exp15_")
    with open(os.path.join(config_dir, "config.json"), "w") as f:
        json.dump(TINY_CONFIG, f)

    model = Llama3TpPartModelFromDir(
