#!/usr/bin/env python3
"""
Experiment 4 — Domain-Specific Routing Variance (Llama3-8B)
============================================================
Tests whether task domain (brainstorming, coding, medical, math, etc.) affects
prediction error and timing variance for the dense Llama3-8B model.

Control baseline: no MoE routing exists in Llama3, so domain should NOT create
domain-specific variance. Any per-domain variance difference here is GPU jitter only,
and can be directly compared against Mixtral EP (test/mixtral/exp4/domain_experiment.py)
where MoE routing may create domain-specific variance.

Dataset: databricks/databricks-dolly-15k (7 domains, Apache 2.0)
  brainstorming, classification, closed_qa, generation,
  information_extraction, open_qa, summarization
Tokenizer: AutoTokenizer.from_pretrained(MODEL_DIR)

Design:
  Phase 0: Load + tokenize dolly-15k by domain (CPU, before model loads)
  Phase 1: Calibration — 256 batches, round-robin across all domains (real tokens)
  Phase 2: Per-domain profiling — up to 25 batches per domain (same-domain batches)
  Phase 3: Mixed profiling — 50 batches with 1 request from each of 4 different domains

Output:
  results/per_batch_domain.csv  — per-batch timing with domain label
  results/domain_summary.csv    — per-domain aggregate stats (mean_ms, cv%, error%)
  results/gate_decisions.csv    — pooled FP/FN at various SLO thresholds

Usage:
    cd S-LoRA
    python test/llama3/exp4/domain_experiment.py
"""
...
BS           = 4
MAX_LEN      = 1024    # max tokens per prompt after truncation
MIN_LEN      = 16      # skip examples shorter than this
N_PER_DOMAIN = 200     # max examples to sample per domain
N_CALIB      = 256     # calibration batches (domain-neutral)
N_PROF       = 25      # profiling batches per domain
N_MIXED      = 50      # cross-domain profiling batches
SFT_BUDGET   = 256
N_WARMUP     = 20
...
def load_and_tokenize():
    """Load dolly-15k, tokenize instructions, group by domain. Returns dict."""
    ...
    for ex in ds:
        ...
        if cat not in by_domain:
            by_domain[cat] = []
        by_domain[cat].append(ids)   # keep as Python list (serializable for mp)

    print(f"\nDomain summary after tokenization:")
...
def build_batches(by_domain):
    """Split each domain into calibration/profiling pools and build batches."""
    ...
    # Calibration: interleave one example per domain, then group into batches of BS=4.
    # Round-robin ensures domain-neutral calibration.
    calib_flat = []
    ...
    # Per-domain profiling batches (same-domain, contiguous)
    domain_batches = {}
    for cat in domains:
        pool    = prof_pool[cat]
        batches = [pool[i:i+BS] for i in range(0, len(pool) - BS + 1, BS)]
        if len(batches) >= 5:
            domain_batches[cat] = batches[:N_PROF]

    # Mixed-domain batches: 1 example from each of the first 4 available domains
    mix_domains = [d for d in domains if d in domain_batches][:4]
    mixed_batches = []
...
def run_experiment(model, vocab_size, calib_batches, domain_batches, mixed_batches):
    from slora.server.router.tracker import PrefillExecutionEstimator

    # Phase 1: Calibration (domain-neutral)
    ...
    # Phase 2: Per-domain profiling (same-domain batches)
...
def print_summary(domain_rows, gate_rows):
    print("\n" + "=" * 76)
    print("DOMAIN ROUTING EXPERIMENT — Llama3-8B (dense, no MoE routing)")
    print("=" * 76)
...
def main():
    ...
    # Phase 0: Data preparation (CPU — before model loads)
    by_domain = load_and_tokenize()
    calib_batches, domain_batches, mixed_batches = build_batches(by_domain)
    ...
    est, per_batch  = run_experiment(model, vocab_size, calib_batches, domain_batches, mixed_batches)
