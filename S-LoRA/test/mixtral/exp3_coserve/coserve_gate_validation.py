#!/usr/bin/env python3
"""
Experiment 3-CoServe — Co-serving Gate Validation (Mixtral-8x7B EP)
=====================================================================
Addresses the core limitation of exp3/variance_profiler.py: that gate
FP/FN classification used inference-only time as ground truth, not the
actual co-serving time (inference + SFT tokens physically together).

This experiment, for each profiling batch, runs TWO forward passes:
...
Ground truth:    actual_coserve_ms ≤ SLO                  (NEW — true co-serving)

This gives unambiguous FP/FN:
  FP = gate admitted SFT, but actual co-serving violated SLO
  FN = gate blocked SFT, but actual co-serving would have fit

Also records:
  - actual_inf_ms vs actual_coserve_ms to measure the cost of adding SFT tokens
  - pred_coserve_ms vs actual_coserve_ms error to measure co-serving prediction accuracy

Requires 2 GPUs and Mixtral-8x7B-v0.1 weights.

Usage:
    cd S-LoRA
    python test/mixtral/exp3_coserve/coserve_gate_validation.py
"""
...
# Same log-normal params as exp3/variance_profiler.py
LN_MU    = 4.5
LN_SIGMA = 1.2
LEN_MIN  = 32
LEN_MAX  = 512
...
SFT_SEED = 7777  # fixed seed for SFT token IDs (reproducible)
...
def run_experiment(model, vocab_size, rank, batches):
    ...
    rng     = np.random.default_rng(999)   # inference token IDs
    sft_rng = np.random.default_rng(SFT_SEED)  # SFT token IDs (separate)

    calib_batches = batches[:N_CALIB]
    prof_batches  = batches[N_CALIB:]

    # -----------------------------------------------------------------------
    # Phase 1: Calibration (inference-only, same as exp3)
    # -----------------------------------------------------------------------
    if rank == 0:
    ...
    for i, lens in enumerate(prof_batches):
        # Draw inference tokens (same seed as exp3 for reproducible comparison)
        inf_tokens = [rng.integers(0, vocab_size, size=l) for l in lens]

        # Draw SFT tokens (fixed separate seed)
...
# ---------------------------------------------------------------------------
# Gate analysis — using actual_coserve_ms as ground truth
# ---------------------------------------------------------------------------

def analyze_gate(per_batch, slo_thresholds_ms):
    """
    True co-serving gate analysis:
      gate_admits = pred_coserve_ms <= SLO
      ground_truth = actual_coserve_ms <= SLO  (NOT inference-only)

    Compare with sim_gate (uses actual_inf_ms as proxy, as in exp3).
    """
    rows = []
    for slo_ms in slo_thresholds_ms:
        # True co-serving FP/FN
        fp_co = fn_co = tp_co = tn_co = 0
        # Simulated (inf-only proxy) FP/FN for comparison
        fp_sim = fn_sim = tp_sim = tn_sim = 0
        admitted = 0

        for r in per_batch:
            gate_admits = r["pred_coserve_ms"] <= slo_ms
            # True ground truth: actual co-serving time
            fits_coserve = r["actual_coserve_ms"] <= slo_ms
            # Sim ground truth (old exp3 proxy): inference-only time
        ...
        rows.append({
            ...
            "n_batches":         n,
            # True co-serving gate performance
            "fp_coserve":        fp_co,
            "fn_coserve":        fn_co,
            "tp_coserve":        tp_co,
            "tn_coserve":        tn_co,
            ...
            # Simulated (inf-only proxy) gate performance
            "fp_sim":            fp_sim,
            "fn_sim":            fn_sim,
            "tp_sim":            tp_sim,
            "tn_sim":            tn_sim,
...
def compute_overhead_stats(per_batch):
    """Summarise co-serving overhead (actual_coserve_ms - actual_inf_ms)."""
...
def print_summary(per_batch, overhead_stats, gate_rows, fit_rmse_ms):
    ...
    print("\n" + "=" * 72)
    print("CO-SERVING GATE VALIDATION — Mixtral-8x7B EP (log-normal trace)")
    print("=" * 72)
    ...
    print(f"\n  Key question: do FP_true and FP_sim agree?")
    print(f"  If FP_sim > FP_true: exp3 was over-counting false positives "
          f"(co-serving actually fits within SLO).")
    print(f"  If FP_sim < FP_true: exp3 was under-counting (co-serving overhead "
          f"causes additional violations beyond inference-only time).")
...
def main():
    ...
    print_summary(per_batch, overhead_stats, gate_rows,
                  fit_rmse_ms=mp_results["est_fit_rmse"] * 1000)
